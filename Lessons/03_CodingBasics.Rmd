...
```{r}
temp_high <- c(45,49,50,52,40,40,45,52,45,52)
temp_low <-  c(29,41,24,23,24,23,26,30,34,45)
```

Now, create two additional vectors that include the ten-day forecast for the high and low temperatures in Celsius.

```{r}
temp_low_c <- round((temp_low-32)*5/9)
temp_high_c <- round((temp_high-32)*5/9)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
temp <- data.frame(temp_high,temp_low,temp_high_c,temp_low_c)
names(temp) <- c("High F","Low F", "High C", "Low C"); temp
